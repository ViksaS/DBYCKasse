...
// Überprüfen, ob "Member" angehakt ist
const isMember = document.getElementById('memberCheckbox').checked;

let finalPrice = price;

// Rabatt nur auf Bier und Kaffee, wenn Mitglied
if (isMember && (name.toLowerCase().includes('x'))) {
finalPrice = (price * 0.9).toFixed(2); // 10% Rabatt
finalPrice = parseFloat(finalPrice);   // wieder zu einer Zahl machen
}

// Prüfen, ob das Produkt schon in der Bestellung ist
const existingItem = order.find(item => item.name === name && item.price === finalPrice);
...
renderOrder(); // Bestellliste neu anzeigen
}
